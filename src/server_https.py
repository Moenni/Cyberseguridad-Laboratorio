...
class CookieHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path == "/login":
         # Mostrar formulario de login
            ...
            try:
                with open("../templates/transferencias.html", "r", encoding="utf-8") as f:
                    contenido = f.read()
    ...
    def do_POST(self):
        ...
        if self.path == "/login":
            ...
            try:
                with open("../templates/transferencias.html","rb")as f:
                  contenido = f.read()
                self.send_response(200)
                self.send_header("Content-type", "text/html; charset=utf-8")
                self.end_headers()
...
def load_key():
  with open("../certs/secret.key","rb")as f:
    return f.read()
key = load_key()
cipher = Fernet(key)
...
context.load_cert_chain(certfile="../certs/localhost+1.pem", keyfile="../certs/localhost+1-key.pem")
